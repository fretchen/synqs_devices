...
class Mako_Camera(object):
    ...
    def get_attributes(self, visibility_level, writeable_only=True):
        ...
        del (
            props["AcquisitionAbort"],
            props["AcquisitionStart"],
            props["AcquisitionStop"],
            props["GVSPAdjustPacketSize"],
            props["GevTimestampControlLatch"],
            props["GevTimestampControlReset"],
            props["LUTLoadAll"],
            props["LUTSaveAll"],
            props["TriggerSoftware"],
            props["UserSetLoad"],
            props["UserSetSave"],
        )
        return props
    ...
    def grab_multiple(self, n_images, images):
        ...
        self.frames = [
            self.camera.new_frame() for _ in range(n_images)
        ]  # Make a frame buffer.
        for self.frame in self.frames:
            self.frame.announce()
        self.camera.start_capture()
